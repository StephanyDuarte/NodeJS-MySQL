...
export class ServerError extends Error{
    static HANDLERS = [sendHandler, logHandler];
...
export function toJSON(err) {
    const type = `${err.statusCode || err.errorCode || err.status || err.code || "Unknown"}`; //500 ou unkown
...
export function logHandler(err, req, res, next) {
    ...
    errorObj.statusCode ===500? error(log) : info(error);

    if (next) next(err);

}

//chrom do server - PESQUISAR?