...
function shortMsg({origin = pjson.name, level, description,
     err=null, timestamp, logger, ...content}){
        const src = origin? `[${origin}]` : ``;
        const msg = `[${formatDate(timestamp)}] ${src}${level}: ${description}`
        if (Object.keys(content).size >0) msg += JSON.stringify(content)
        if (err) {
            if (err.stack) msg += `\n${err.stack}`
...
function longMsg({origin = pjson.name, level, description,
     err=null, timestamp, logger, ...content}){
        //TODO
     }
...
//funcao de echo e a funcao longa